...
#@jax.jit(static_argnums=(0, 1, 2, 3, 4, 11, 15))
def Qsc_method(rc, zs, rs=(1, 0.09), zc=(0, -0.09), nfp=1, etabar=1., sigma0=0., B0=1., I2=0., sG=1, spsi=1, nphi=61, B2s=0., B2c=0., p2=0., order="r1") -> Results:
  ...
  find_max = np.array([len(rc), len(zs), len(rs), len(zc)])
  nfourier = np.max(find_max)

  if np.mod(nphi, 2) == 0:
            nphi += 1

  
  pre_calculations_results, calculation_results = computer_core(rc, zs, rs, zc, nfp, etabar, sigma0, B0, I2, sG, spsi, nphi, B2s, order, nfourier, B2c, p2)
...
jax.jit()
def computer_core(rc_raw, zs_raw, rs_raw, zc_raw, nfp, etabar, sigma0, B0, I2, sG, spsi, nphi, B2s, order, nfourier, B2c, p2):
    ...
    rc = rc.at[:len(rc_raw)].set(rc_raw)
    zs = zs.at[:len(zs_raw)].set(zs_raw)
    rs = rs.at[:len(rs_raw)].set(rs_raw)
    zc = zc.at[:len(zc_raw)].set(zc_raw)

    pre_calculations_results = pre_calculations(rc, zs, rs, zc, nfp, etabar, sigma0, B0, I2, sG, spsi, nphi, B2s, order,
                                            nfourier)
